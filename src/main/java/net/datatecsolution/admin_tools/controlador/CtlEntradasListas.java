...
public class CtlEntradasListas implements ActionListener, MouseListener {
	...
	public CtlEntradasListas(ViewListaEntradas v){
		view=v;
	...
	private void cargarTabla(List<EntradaCaja> entradas) {
		// TODO Auto-generated method stub
		view.getModelo().limpiar();
		if(entradas!=null){
		for(int x=0;x<entradas.size();x++)
			view.getModelo().agregar(entradas.get(x));
		}
	}
	...
	public boolean verificarSelecion(){
		...
		if(view.getTabla().getSelectedRow()!=-1){
			...
			this.myEntrada=view.getModelo().getEntrada(filaPulsada);
			resul=true;
		}else{
		...
		//se recoge el comando programado en el boton que se hizo click
				...
				case "ELIMINAR":
					...
					if(verificarSelecion()){
									...
									int resul=JOptionPane.showConfirmDialog(view, "Desea "+((myEntrada.getEstado().equals("ACT")?"NULA":"ACT"))  +" la salida no "+this.myEntrada.getCodigoEntrada()+"?");
									...
									if(resul==0){
										...
										if(action < 0){
											...
											if(this.myUsuarioDao.comprobarAdmin(pwd)){
												...
												myEntrada.setEstado((myEntrada.getEstado().equals("ACT"))?"NULA":"ACT");
												
												//se anula la factura en la bd
												if(this.myDao.actualizarEstado(myEntrada)){
				...
				case "BUSCAR":
					...
					if(this.view.getRdbtnId().isSelected()){
						...
						this.myEntrada=myDao.buscarPorId(Integer.parseInt(view.getTxtBuscar().getText()));
						if(myEntrada!=null){
							view.getModelo().limpiar();
							view.getModelo().agregar(myEntrada);
						}
      ...
      //si fue doble click mostrar modificar
    	if (e.getClickCount() == 2) {
    		...
    		try {
				...
				AbstractJasperReports.createReportEntradaCaja(ConexionStatic.getPoolConexion().getConnection(), myEntrada.getCodigoEntrada());
