...
public class FacturaDao extends ModeloDaoBasic {
	...
	public String getFechaSistema() {
		...
		try {
			...
			// res=getFecha.executeQuery(" SELECT DATE_FORMAT(now(), '%d/%m/%Y %h:%i:%s %p')
			// as fecha;");
	...
	public boolean registrar(Object c) {
		...
		try {
			...
			if (myFactura.getTipoFactura() == 2) {
				...
				// verificar si tiene credito a fovor del cliente, sino tiene > 0 se registra el
				// credito de la factura
				if (ultima.getSaldo().doubleValue() < 0) {
					...
					if (newSaldo.doubleValue() <= 0) {
						...
						cuenta.setSaldo(new BigDecimal(myFactura.getTotal()
	...
	/*
	 * <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Metodo para seleccionar las facturas por
	 * id>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	 */
	...
	/*
	 * <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Metodo para seleccionar todos los
	 * articulos>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	 */
	...
	public List<Factura> buscarPorFecha(String fecha1, String fecha2, int limitInferio, int canItemPag, Object c) {
		...
		try {
			...
			psConsultas = con.prepareStatement(super.getQuerySearchFecha("DATE_FORMAT(fecha,'%y-%m-%d')",
	...
	public List<Factura> sinPagarCliente(Cliente myCliente) {
		...
		try {
			...
			psConsultas = con.prepareStatement(super.getQuerySelect()
					+ " where encabezado_factura.codigo_cliente=? and encabezado_factura.estado_factura = 'ACT' and encabezado_factura.estado_pago = 0");
	...
	public Factura getUltimaFacturaUser(String user, Caja caja) {
		...
		String sql = super.getQuerySelect()
				+ " where encabezado_factura.usuario=? ORDER BY encabezado_factura.numero_factura DESC limit 1";
		// Statement stmt = null;
		Factura unaFactura = new Factura();
	...
	public List<Factura> buscarPorVendedor(String nombre, int limitInferio, int canItemPag, Object c) {
		...
		try {
			...
			psConsultas = con.prepareStatement(
					super.getQuerySearchJoin("nombre", "LIKE", "empleados", "codigo_empleado", "codigo_vendedor"));
	...
	public List<Factura> buscarPorNombreCliente(String nombre, int limitInferio, int canItemPag, Object c) {
		...
		try {
			...
			psConsultas = con.prepareStatement(
					super.getQuerySearchJoin("nombre_cliente", "LIKE", "cliente", "codigo_cliente", "codigo_cliente"));
	...
	public boolean verificarCierre(Caja caja, CierreFacturacion registroFacturasCaja) {
		...
		String sql = super.getQuerySelect()
				+ " WHERE	encabezado_factura.numero_factura >= ? AND encabezado_factura.usuario = ? ";
	...
	public void calcularCierre(Caja caja, CierreFacturacion registroFacturas, String usuario, CierreCaja unaCierre) {
		...
		try {
			...
			while (res.next()) {
				...
				// unaCierre.setNoSalidaInicial(ultimoCierreUser.getNoSalidaInicial());/// este
				// es el error
				...
				// BigDecimal
				// t_efectivo=ultimoCierreUser.getEfectivoInicial().add(res.getBigDecimal("total_efectivo"));//se
				// suma el efectivo inicial y la venta con efectivo
	...
	public void getComiciones(String fecha1, String fecha2, Caja caja, List<Comision> comisiones,
		...
		try {
			...
			while (res.next()) {
				int itemEmpleado = -1;
				// se revisa la lista de los empleados cargados previos establecer a quien
				// corresponde el resultado obtenido
	...
	public void getVentasUsuarios(String fecha1, String fecha2, Caja caja, List<Comision> comisiones,
			int comisionPorcentaje) {
		...
		try {
			...
			e.printStackTrace();
			// JOptionPane.showMessageDialog(null, e.getMessage(),"Error en la base de
	...
	public void getVentasCategorias(Integer noFacturaIncial, Integer noFacturaFinal, String usuario, Caja caja,
			List<VentasCategoria> ventas) {
