...
-- US-133: STUB para instalacion fresca. La version real de `articulo_view`
-- (que invoca funciones creadas recien en V7) se recrea al FINAL de esta baseline (las funciones nacen mas abajo).
-- En clientes ya migrados este cambio no re-ejecuta; repair() realinea checksum.
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `articulo_view` AS select `articulo`.`codigo_articulo` AS `codigo_articulo`,`articulo`.`articulo` AS `articulo`,`articulo`.`codigo_marca` AS `codigo_marca`,`articulo`.`cod_articulo` AS `cod_articulo`,`articulo`.`codigo_impuesto` AS `codigo_impuesto`,`articulo`.`precio_articulo` AS `precio_articulo`,`articulo`.`tipo_articulo` AS `tipo_articulo`,`articulo`.`estado` AS `estado`,ifnull(0,0) AS `existencia` from `articulo` where (`articulo`.`estado` = 1);
...
-- US-133: STUB para instalacion fresca. La version real de `v_proveedores`
-- (que invoca funciones creadas recien en V7) se recrea al FINAL de esta baseline (las funciones nacen mas abajo).
-- En clientes ya migrados este cambio no re-ejecuta; repair() realinea checksum.
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_proveedores` AS select 0 AS `saldo`,`proveedor`.`codigo_proveedor` AS `codigo_proveedor`,`proveedor`.`nombre_proveedor` AS `nombre_proveedor`,`proveedor`.`telefono` AS `telefono`,`proveedor`.`celular` AS `celular`,`proveedor`.`direccion` AS `direccion` from `proveedor`;
...
-- =====================================================================
-- US-133: recreacion REAL de las vistas que arriba quedaron como STUB.
-- La baseline generada creaba estas 3 vistas ANTES que las funciones
-- que invocan (f_existencia_y_ordenes, f_saldo_cliente,
-- f_saldo_proveedor, definidas mas abajo): MySQL valida la funcion en
-- el CREATE VIEW y la instalacion FRESCA abortaba en V1 (descubierto
-- aprovisionando el stack mariposas, 2026-08-05). Aqui, al final, las
-- funciones ya existen. En clientes ya migrados V1 nunca re-ejecuta;
-- el repair() del arranque realinea el checksum (precedente V36).
-- =====================================================================
