SELECT YEAR(v.fecha) AS anio, m.descripcion AS categoria,
       ROUND(SUM(v.cantidad),0) AS unidades, ROUND(SUM(v.total),2) AS total_lps
  FROM (
    SELECT e.fecha, d.codigo_articulo, d.cantidad, d.total
     ...
     WHERE e.estado_factura <> 'NULA'
       AND ((e.fecha >= '2025-07-01' AND e.fecha < '2025-07-19') OR (e.fecha >= '2026-07-01' AND e.fecha < '2026-07-19'))
    UNION ALL
    SELECT e.fecha, d.codigo_articulo, d.cantidad, d.total
 ...
 GROUP BY anio, m.codigo_marca, m.descripcion
 ORDER BY anio, total_lps DESC;
