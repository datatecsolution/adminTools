...
- **Front (queda igual)**: ⚠ **CORREGIDO 2026-07-30 tras el deploy** — esta afirmación se hizo leyendo el CÓDIGO FUENTE del front, que ya había migrado a `/products/description` y `/customers`. **El BUILD DESPLEGADO en Sharon estaba atrás** y seguía llamando a `/products/despriciouser/{d}` y `/costomers/name/{n}`, borrados en US-022 → búsqueda de productos y clientes rota tras el deploy (404). Resuelto con `LegacyOrdersAppCtl` (US-123, imagen `sharon-v43`). **REGLA para venecia/dulce: verificar el contrato contra el BUILD EN PRODUCCIÓN (logs del proxy o el bundle servido), nunca contra el repo del front.** Fuera de eso, los campos nuevos sí son aditivos. Si algún día se activa el bloqueo de stock para un vendedor, el front viejo muestra error genérico en vez del detalle bonito — degradación aceptable, no rotura.
